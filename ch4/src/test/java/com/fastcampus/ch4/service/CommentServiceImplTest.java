...
public class CommentServiceImplTest {
    ...
    public void remove() throws Exception {
        boardDao.deleteAll();   // 게시판 데이터 모두 삭제

        BoardDto boardDto = new BoardDto("hello", "hello", "asdf");
        assertTrue(boardDao.insert(boardDto) == 1); // 게시글 1개 추가
        Integer bno = boardDao.selectAll().get(0).getBno(); // 첫 번째 게시글 bno 가져오기
        System.out.println("bno = " + bno);

        commentDao.deleteAll(bno);  // bno에 해당하는 댓글 모두 삭제
